...
class Bett3R(nn.Module):
    def __init__(self, mast3r_name="./checkpoints/MASt3R_ViTLarge_BaseDecoder_512_dpt.pth", 
                 use_feat=False, mem_pos_enc=False, memory_dropout=0.15):
        super(Bett3R, self).__init__()
        # config
        self.use_feat = use_feat
        self.mem_pos_enc = mem_pos_enc

        # MASt3R
        self.mast3r = AsymmetricMASt3R.from_pretrained(mast3r_name, landscape_only=True)
    ...
    def set_memory_encoder(self, enc_depth=6, enc_embed_dim=1024, out_dim=1024, enc_num_heads=16, 
        ...
        self.value_encoder = nn.ModuleList([
            Block(enc_embed_dim, enc_num_heads, mlp_ratio, qkv_bias=True, 
                  norm_layer=norm_layer, rope=self.mast3r.rope if self.mem_pos_enc else None)
        ...
        if not self.use_feat:
            self.pos_patch_embed = copy.deepcopy(self.mast3r.patch_embed)
            self.pos_patch_embed.load_state_dict(self.mast3r.patch_embed.state_dict())
    ...
    def encode_image(self, view):
        ...
        out, pos, _ = self.mast3r._encode_image(img, im_shape)
    ...
    def encode_image_pairs(self, view1, view2):
        ...
        out, pos, _ = self.mast3r._encode_image(torch.cat((img1, img2), dim=0),
    ...
    def downstream_head(self, dec, true_shape, num=1):
        with torch.cuda.amp.autocast(enabled=False):
            res = self.mast3r._downstream_head(num, [tok.float() for tok in dec], true_shape)
